...
Build a feature with exceptional UX and UI quality through a structured process: shape the design, load the right references, then build and iterate visually until the result is delightful.

## Step 1: Shape the Design

Run `/forgia`, passing along whatever feature description the user provided.
...
If the user has already run `/forgia` and has a confirmed design brief, skip this step and use the existing brief.
...
- `~/.claude/reference/spatial-design.md` for layout and spacing
- `~/.claude/reference/typography.md` for type hierarchy

Then add references based on the brief's needs:
- Complex interactions or forms? Consult `~/.claude/reference/interaction-design.md`
- Animation or transitions? Consult `~/.claude/reference/motion-design.md`
- Color-heavy or themed? Consult `~/.claude/reference/color-and-contrast.md`
- Responsive requirements? Consult `~/.claude/reference/responsive-design.md`
- Heavy on copy, labels, or errors? Consult `~/.claude/reference/ux-writing.md`
...
3. **Check against the DON'T guidelines** in the magistero command. Fix any anti-pattern violations.
