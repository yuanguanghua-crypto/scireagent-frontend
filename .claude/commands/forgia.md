Plan the UX and UI for a feature before writing code. Runs a structured discovery interview, then produces a design brief that guides implementation.

Feature to shape: $ARGUMENTS

## MANDATORY PREPARATION

1. **Design context**: Read `.design-context.md` from the project root. If it doesn't exist, ask the user to run `/magistero teach` first — do not proceed without it.
2. **Design principles**: Read `~/.claude/commands/magistero.md` and internalize its design guidelines, anti-pattern rules, and the AI Slop Test. Follow them throughout.

---

**Scope**: Design planning only. This skill does NOT write code. It produces the thinking that makes code good.

**Output**: A design brief that can be handed off to `/magistero craft` or any other implementation skill.
...
Ask these questions in conversation, adapting based on answers. Don't dump them all at once; have a natural dialogue. Ask the user.
...
**3. Design Direction**
How this should feel. What aesthetic approach fits. Reference the project's design context from `.design-context.md` and explain how this feature should express it.
...
Based on the brief, list which reference files would be most valuable during implementation (e.g., `.claude/reference/spatial-design.md` for complex layouts, `.claude/reference/motion-design.md` for animated features, `.claude/reference/interaction-design.md` for form-heavy features).
...
Once confirmed, the brief is complete. The user can now hand it to `/magistero craft` to build the feature, or use it to guide any other implementation approach.
