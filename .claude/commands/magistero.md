...
If `$ARGUMENTS` is "craft", follow the **Craft Mode** section at the end. If `$ARGUMENTS` is "teach", follow the **Teach Mode** section at the end. Otherwise, use `$ARGUMENTS` as the target and follow the full design creation flow below.
...
2. **Check .design-context.md (fast)**: If not in instructions, read `.design-context.md` from the project root. If it exists and contains the required context, proceed.
3. **Run teach mode (REQUIRED)**: If neither source has context, you MUST run `/magistero teach` NOW before doing anything else. Do NOT skip this step. Do NOT attempt to infer context from the codebase instead.
...
### Typography
→ *Consult the typography reference (`.claude/reference/typography.md`) for OpenType features, web font loading, and the deeper material on scales.*
...
### Color & Theme
→ *Consult the color reference (`.claude/reference/color-and-contrast.md`) for the deeper material on contrast, accessibility, and palette construction.*
...
### Layout & Space
→ *Consult the spatial reference (`.claude/reference/spatial-design.md`) for the deeper material on grids, container queries, and optical adjustments.*
...
### Motion
→ *Consult the motion reference (`.claude/reference/motion-design.md`) for timing, easing, and reduced motion.*
...
### Interaction
→ *Consult the interaction reference (`.claude/reference/interaction-design.md`) for forms, focus, and loading patterns.*
...
### Responsive
→ *Consult the responsive reference (`.claude/reference/responsive-design.md`) for mobile-first, fluid design, and container queries.*
...
### UX Writing
→ *Consult the ux-writing reference (`.claude/reference/ux-writing.md`) for labels, errors, and empty states.*
...
If this command was invoked with "craft" (e.g., `/magistero craft [feature description]`), read and follow the craft flow in `.claude/reference/craft.md`. Pass any additional arguments as the feature description.

---

## Teach Mode

If this command was invoked with "teach" (e.g., `/magistero teach`), skip all design work above and instead run the teach flow below. This is a one-time setup that gathers design context for the project.
...
Write this section to `.design-context.md` in the project root. If the file already exists, update the Design Context section in place.

Then ask the user whether they'd also like the Design Context appended to `CLAUDE.md`. If yes, append or update the section there as well.
