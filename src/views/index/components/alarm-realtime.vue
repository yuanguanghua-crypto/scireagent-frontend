...
<div class="box">
    ...
    <!-- <vue-seamless-scroll :data="dataArr" class="seamless-warp" :class-option="classOption"> -->
        <div class="warning-box" v-for="(item,index) in dataArr" :key="index">
            <el-row :gutter="20">
                <el-col :span="8">
                    <img v-if="item.picBase64" class="el-image" :src="getBaseImgs(item.picBase64)" alt=""></img>
                ...
                <el-col :span="16">
                    <ul style="padding:5px 0">
                        <!-- -- danger  success -- -->
                        <li class="alarm_item">告警类型：{{item.sourceName}}</li>
                        <li class="alarm_item">时间：{{getTimeFormat(item.createTime) }}</li>
                        <li class="alarm_item">                            
                            <tooltip-over :content="'告警来源：'+item.channelName" class="190" refName="tooltipOver"></tooltip-over> 
                        </li>
                    </ul>
                </el-col>
            </el-row>
...
import { realtimeAlarmTable } from '@/api/v3/largeScreen/set.js';
...
export default {
    ...
    methods: {
        ...
        initData(){
            ...
            realtimeAlarmTable(this.sourceCodeList).then(res =>{
                if(res.code == 1000){
        ...
        appendWarning(msgParam) {
            console.log('--推送---实时告警-----------',msgParam);
            if(msgParam.eventSource && this.sourceCodeList.includes(msgParam.sourceCode)){
                let msgParams = {}
